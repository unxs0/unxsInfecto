	...
	if(!strcmp(gcFunction,"tMedicoTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tMedico(LANG_NB_CONFIRMNEW);
			}
			else
				tMedico("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tMedico Aux Panel",100);
	switch(guMode)
        {
                case 2000:
		...
		default:
			...
			printf("Tabla de medicos. Usado en consultas y interconsultas.<br>(RADBuildInfo %.32s)\n",sgcBuildInfo);
...
void ExttMedicoGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uMedico"))
		{
			sscanf(gentries[i].val,"%u",&uMedico);
			guMode=6;
		}
	}
	tMedico("");
...
void ExttMedicoSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tMedico ORDER BY"
				" uMedico",
				VAR_LIST_tMedico);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tMedico,tClient WHERE tMedico.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uMedico",
					VAR_LIST_tMedico,uContactParentCompany,uContactParentCompany);
...
void ExttMedicoSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tMedico WHERE uMedico=%u",
			VAR_LIST_tMedico,uMedico);
	else
                sprintf(gcQuery,"SELECT %s FROM tMedico,tClient"
                                " WHERE tMedico.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tMedico.uMedico=%u",
                        		VAR_LIST_tMedico
...
void ExttMedicoListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tMedico",
				VAR_LIST_tMedico);
	else
		sprintf(gcQuery,"SELECT %s FROM tMedico,tClient"
				" WHERE tMedico.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttMedicoListFilter()
        if(!strcmp(gcFilter,"uMedico"))
        {
                sscanf(gcCommand,"%u",&uMedico);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tMedico.uMedico=%u"
						" ORDER BY uMedico",
						uMedico);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uMedico");
        }
...
void ExttMedicoListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uMedico"))
                printf("<option>uMedico</option>");
        else
                printf("<option selected>uMedico</option>");
...
void tMedicoNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uMedico,cLabel FROM tMedico ORDER BY cLabel");
	else
        ...
        {
        	printf("<p><u>tMedicoNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tMedico"
				"&uMedico=%s>%s</a><br>\n",
				field[0],field[1]);
	}
