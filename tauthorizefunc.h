		...
		default:
			...
			if(uCertClient>1 && uOwner>1)
				printf("This login appears to belong to a"
					" <a class=darkLink href=unxsSalud.cgi?gcFunction=tClient&uClient=%u>'%s'</a> company contact"
					" '<a class=darkLink href=unxsSalud.cgi?gcFunction=tClient&uClient=%u>%s</a>'.<br>"
...
void tAuthorizeNavList(void)
{
	...
	if(mysql_num_rows(res))
	{
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tAuthorize&uAuthorize=%s>%s/%s/%s</a><br>\n",
