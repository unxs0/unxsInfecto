/*
FILE
	tClient source code of unxsSalud.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetClient();
				unxsSalud("New tClient table created");
                	}
			else
			{
...
void NewtClient(unsigned uMode)
{
	...
	unxsSaludLog(uClient,"tClient","New");
#endif
...
void DeletetClient(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uClient,"tClient","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uClient,"tClient","DelError");
#endif
...
void ModtClient(void)
{
	...
	unxsSaludLog(uClient,"tClient","Mod");
#endif
	tClient(gcQuery);
