/*
FILE
	tLogType source code of unxsSalud.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetLogType();
				unxsSalud("New tLogType table created");
                	}
			else
			{
...
void NewtLogType(unsigned uMode)
{
	...
	unxsSaludLog(uLogType,"tLogType","New");
#endif
...
void DeletetLogType(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uLogType,"tLogType","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uLogType,"tLogType","DelError");
#endif
...
void ModtLogType(void)
{
	...
	unxsSaludLog(uLogType,"tLogType","Mod");
#endif
	tLogType(gcQuery);
