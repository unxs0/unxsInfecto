	...
	if(!strcmp(gcFunction,"tConsultaTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tConsulta(LANG_NB_CONFIRMNEW);
			}
			else
				tConsulta("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tConsulta Aux Panel",100);
	switch(guMode)
        {
                case 2000:
		...
		default:
			...
			printf("Tabla de consultas. Un paciente puede tener 0 o mas consultas.<br>(RADBuildInfo %.32s)\n",sgcBuildInfo);
...
void ExttConsultaGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uConsulta"))
		{
			sscanf(gentries[i].val,"%u",&uConsulta);
			guMode=6;
		}
	}
	tConsulta("");
...
void ExttConsultaSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tConsulta ORDER BY"
				" uConsulta",
				VAR_LIST_tConsulta);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tConsulta,tClient WHERE tConsulta.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uConsulta",
					VAR_LIST_tConsulta,uContactParentCompany,uContactParentCompany);
...
void ExttConsultaSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tConsulta WHERE uConsulta=%u",
			VAR_LIST_tConsulta,uConsulta);
	else
                sprintf(gcQuery,"SELECT %s FROM tConsulta,tClient"
                                " WHERE tConsulta.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tConsulta.uConsulta=%u",
                        		VAR_LIST_tConsulta
...
void ExttConsultaListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tConsulta",
				VAR_LIST_tConsulta);
	else
		sprintf(gcQuery,"SELECT %s FROM tConsulta,tClient"
				" WHERE tConsulta.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttConsultaListFilter()
        if(!strcmp(gcFilter,"uConsulta"))
        {
                sscanf(gcCommand,"%u",&uConsulta);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tConsulta.uConsulta=%u"
						" ORDER BY uConsulta",
						uConsulta);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uConsulta");
        }
...
void ExttConsultaListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uConsulta"))
                printf("<option>uConsulta</option>");
        else
                printf("<option selected>uConsulta</option>");
...
void tConsultaNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uConsulta,cLabel FROM tConsulta ORDER BY cLabel");
	else
        ...
        {
        	printf("<p><u>tConsultaNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tConsulta"
				"&uConsulta=%s>%s</a><br>\n",
				field[0],field[1]);
	}
