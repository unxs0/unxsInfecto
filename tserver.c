/*
FILE
	tServer source code of unxsSalud.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetServer();
				unxsSalud("New tServer table created");
                	}
			else
			{
...
void NewtServer(unsigned uMode)
{
	...
	unxsSaludLog(uServer,"tServer","New");
#endif
...
void DeletetServer(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uServer,"tServer","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uServer,"tServer","DelError");
#endif
...
void ModtServer(void)
{
	...
	unxsSaludLog(uServer,"tServer","Mod");
#endif
	tServer(gcQuery);
