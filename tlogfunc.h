	...
	if(uLog && cLabel[0] && guPermLevel>10 && uTablePK[0] && cTableName[0])
	{
		...
		else if(!strcmp(cTableName,"tResource"))
		{
			...
			if( strcmp(cLabel,"Del") && uTPK && uZone && uRRType)
			{
				...
				printf("<a title='Jump to tResource entry' href=unxsSalud.cgi?gcFunction=tResource&uResource=%u>tResource</a><blockquote>\n",uTPK);
			...
			if(uLoginClient)
			{
				printf("Contact=<a title='Jump to tClient entry' href=unxsSalud.cgi?gcFunction=tClient&uClient=%u>%s</a><br></blockquote>\n",uLoginClient,
