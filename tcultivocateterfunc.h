...
void tCultivoCateterNavList(void);
	...
	if(!strcmp(gcFunction,"tCultivoCateterTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tCultivoCateter(LANG_NB_CONFIRMNEW);
			}
			else
				tCultivoCateter("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
	                        guMode=2001;
				tCultivoCateter(LANG_NB_CONFIRMDEL);
			}
			else
				tCultivoCateter("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
				guMode=2002;
				tCultivoCateter(LANG_NB_CONFIRMMOD);
			}
			else
				tCultivoCateter("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tCultivoCateter Aux Panel",100);
	switch(guMode)
        {
                case 2000:
...
void ExttCultivoCateterGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uCultivoCateter"))
		{
			sscanf(gentries[i].val,"%u",&uCultivoCateter);
			guMode=6;
		}
	}
	tCultivoCateter("");
...
void ExttCultivoCateterSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tCultivoCateter ORDER BY"
				" uCultivoCateter",
				VAR_LIST_tCultivoCateter);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tCultivoCateter,tClient WHERE tCultivoCateter.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uCultivoCateter",
					VAR_LIST_tCultivoCateter,uContactParentCompany,uContactParentCompany);
...
void ExttCultivoCateterSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tCultivoCateter WHERE uCultivoCateter=%u",
			VAR_LIST_tCultivoCateter,uCultivoCateter);
	else
                sprintf(gcQuery,"SELECT %s FROM tCultivoCateter,tClient"
                                " WHERE tCultivoCateter.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tCultivoCateter.uCultivoCateter=%u",
                        		VAR_LIST_tCultivoCateter
...
void ExttCultivoCateterListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tCultivoCateter",
				VAR_LIST_tCultivoCateter);
	else
		sprintf(gcQuery,"SELECT %s FROM tCultivoCateter,tClient"
				" WHERE tCultivoCateter.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttCultivoCateterListFilter()
        if(!strcmp(gcFilter,"uCultivoCateter"))
        {
                sscanf(gcCommand,"%u",&uCultivoCateter);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tCultivoCateter.uCultivoCateter=%u"
						" ORDER BY uCultivoCateter",
						uCultivoCateter);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uCultivoCateter");
        }
...
void ExttCultivoCateterListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uCultivoCateter"))
                printf("<option>uCultivoCateter</option>");
        else
                printf("<option selected>uCultivoCateter</option>");
...
void tCultivoCateterNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uCultivoCateter,cLabel FROM tCultivoCateter ORDER BY cLabel");
	else
		sprintf(gcQuery,"SELECT tCultivoCateter.uCultivoCateter,"
				" tCultivoCateter.cLabel"
        ...
        {
        	printf("<p><u>tCultivoCateterNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tCultivoCateter"
				"&uCultivoCateter=%s>%s</a><br>\n",
				field[0],field[1]);
	}
