...
void tCultivoEsputoNavList(void);
	...
	if(!strcmp(gcFunction,"tCultivoEsputoTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tCultivoEsputo(LANG_NB_CONFIRMNEW);
			}
			else
				tCultivoEsputo("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
	                        guMode=2001;
				tCultivoEsputo(LANG_NB_CONFIRMDEL);
			}
			else
				tCultivoEsputo("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
				guMode=2002;
				tCultivoEsputo(LANG_NB_CONFIRMMOD);
			}
			else
				tCultivoEsputo("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tCultivoEsputo Aux Panel",100);
	switch(guMode)
        {
                case 2000:
...
void ExttCultivoEsputoGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uCultivoEsputo"))
		{
			sscanf(gentries[i].val,"%u",&uCultivoEsputo);
			guMode=6;
		}
	}
	tCultivoEsputo("");
...
void ExttCultivoEsputoSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tCultivoEsputo ORDER BY"
				" uCultivoEsputo",
				VAR_LIST_tCultivoEsputo);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tCultivoEsputo,tClient WHERE tCultivoEsputo.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uCultivoEsputo",
					VAR_LIST_tCultivoEsputo,uContactParentCompany,uContactParentCompany);
...
void ExttCultivoEsputoSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tCultivoEsputo WHERE uCultivoEsputo=%u",
			VAR_LIST_tCultivoEsputo,uCultivoEsputo);
	else
                sprintf(gcQuery,"SELECT %s FROM tCultivoEsputo,tClient"
                                " WHERE tCultivoEsputo.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tCultivoEsputo.uCultivoEsputo=%u",
                        		VAR_LIST_tCultivoEsputo
...
void ExttCultivoEsputoListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tCultivoEsputo",
				VAR_LIST_tCultivoEsputo);
	else
		sprintf(gcQuery,"SELECT %s FROM tCultivoEsputo,tClient"
				" WHERE tCultivoEsputo.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttCultivoEsputoListFilter()
        if(!strcmp(gcFilter,"uCultivoEsputo"))
        {
                sscanf(gcCommand,"%u",&uCultivoEsputo);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tCultivoEsputo.uCultivoEsputo=%u"
						" ORDER BY uCultivoEsputo",
						uCultivoEsputo);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uCultivoEsputo");
        }
...
void ExttCultivoEsputoListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uCultivoEsputo"))
                printf("<option>uCultivoEsputo</option>");
        else
                printf("<option selected>uCultivoEsputo</option>");
...
void tCultivoEsputoNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uCultivoEsputo,cLabel FROM tCultivoEsputo ORDER BY cLabel");
	else
		sprintf(gcQuery,"SELECT tCultivoEsputo.uCultivoEsputo,"
				" tCultivoEsputo.cLabel"
        ...
        {
        	printf("<p><u>tCultivoEsputoNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tCultivoEsputo"
				"&uCultivoEsputo=%s>%s</a><br>\n",
				field[0],field[1]);
	}
