...
void tInmunizacionNavList(void);
	...
	if(!strcmp(gcFunction,"tInmunizacionTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tInmunizacion(LANG_NB_CONFIRMNEW);
			}
			else
				tInmunizacion("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
	                        guMode=2001;
				tInmunizacion(LANG_NB_CONFIRMDEL);
			}
			else
				tInmunizacion("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
				guMode=2002;
				tInmunizacion(LANG_NB_CONFIRMMOD);
			}
			else
				tInmunizacion("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tInmunizacion Aux Panel",100);
	switch(guMode)
        {
                case 2000:
...
void ExttInmunizacionGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uInmunizacion"))
		{
			sscanf(gentries[i].val,"%u",&uInmunizacion);
			guMode=6;
		}
	}
	tInmunizacion("");
...
void ExttInmunizacionSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tInmunizacion ORDER BY"
				" uInmunizacion",
				VAR_LIST_tInmunizacion);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tInmunizacion,tClient WHERE tInmunizacion.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uInmunizacion",
					VAR_LIST_tInmunizacion,uContactParentCompany,uContactParentCompany);
...
void ExttInmunizacionSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tInmunizacion WHERE uInmunizacion=%u",
			VAR_LIST_tInmunizacion,uInmunizacion);
	else
                sprintf(gcQuery,"SELECT %s FROM tInmunizacion,tClient"
                                " WHERE tInmunizacion.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tInmunizacion.uInmunizacion=%u",
                        		VAR_LIST_tInmunizacion
...
void ExttInmunizacionListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tInmunizacion",
				VAR_LIST_tInmunizacion);
	else
		sprintf(gcQuery,"SELECT %s FROM tInmunizacion,tClient"
				" WHERE tInmunizacion.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttInmunizacionListFilter()
        if(!strcmp(gcFilter,"uInmunizacion"))
        {
                sscanf(gcCommand,"%u",&uInmunizacion);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tInmunizacion.uInmunizacion=%u"
						" ORDER BY uInmunizacion",
						uInmunizacion);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uInmunizacion");
        }
...
void ExttInmunizacionListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uInmunizacion"))
                printf("<option>uInmunizacion</option>");
        else
                printf("<option selected>uInmunizacion</option>");
...
void tInmunizacionNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uInmunizacion,cLabel FROM tInmunizacion ORDER BY cLabel");
	else
		sprintf(gcQuery,"SELECT tInmunizacion.uInmunizacion,"
				" tInmunizacion.cLabel"
        ...
        {
        	printf("<p><u>tInmunizacionNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tInmunizacion"
				"&uInmunizacion=%s>%s</a><br>\n",
				field[0],field[1]);
	}
