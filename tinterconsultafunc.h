...
void tInterconsulta(const char *results);
void ProcesstInterconsultaVars(pentry entries[], int x);
void tInterconsultaContent(void);
void tInterconsultaInputContent(void);
...
void tInterconsultaNavList(void);
	...
	if(!strcmp(gcFunction,"tInterconsultaTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tInterconsulta(LANG_NB_CONFIRMNEW);
			}
			else
				tInterconsulta("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
	                        guMode=2001;
				tInterconsulta(LANG_NB_CONFIRMDEL);
			}
			else
				tInterconsulta("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
				guMode=2002;
				tInterconsulta(LANG_NB_CONFIRMMOD);
			}
			else
				tInterconsulta("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tInterconsulta Aux Panel",100);
	switch(guMode)
        {
                case 2000:
...
void ExttInterconsultaGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uInterconsulta"))
		{
			sscanf(gentries[i].val,"%u",&uInterconsulta);
			guMode=6;
		}
	}
	tInterconsulta("");
...
void ExttInterconsultaSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tInterconsulta ORDER BY"
				" uInterconsulta",
				VAR_LIST_tInterconsulta);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tInterconsulta,tClient WHERE tInterconsulta.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uInterconsulta",
					VAR_LIST_tInterconsulta,uContactParentCompany,uContactParentCompany);
...
void ExttInterconsultaSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tInterconsulta WHERE uInterconsulta=%u",
			VAR_LIST_tInterconsulta,uInterconsulta);
	else
                sprintf(gcQuery,"SELECT %s FROM tInterconsulta,tClient"
                                " WHERE tInterconsulta.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tInterconsulta.uInterconsulta=%u",
                        		VAR_LIST_tInterconsulta
...
void ExttInterconsultaListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tInterconsulta",
				VAR_LIST_tInterconsulta);
	else
		sprintf(gcQuery,"SELECT %s FROM tInterconsulta,tClient"
				" WHERE tInterconsulta.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttInterconsultaListFilter()
        if(!strcmp(gcFilter,"uInterconsulta"))
        {
                sscanf(gcCommand,"%u",&uInterconsulta);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tInterconsulta.uInterconsulta=%u"
						" ORDER BY uInterconsulta",
						uInterconsulta);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uInterconsulta");
        }
...
void ExttInterconsultaListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uInterconsulta"))
                printf("<option>uInterconsulta</option>");
        else
                printf("<option selected>uInterconsulta</option>");
...
void tInterconsultaNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uInterconsulta,cLabel FROM tInterconsulta ORDER BY cLabel");
	else
		sprintf(gcQuery,"SELECT tInterconsulta.uInterconsulta,"
				" tInterconsulta.cLabel"
        ...
        {
        	printf("<p><u>tInterconsultaNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tInterconsulta"
				"&uInterconsulta=%s>%s</a><br>\n",
				field[0],field[1]);
	}
