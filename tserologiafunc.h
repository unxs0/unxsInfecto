	...
	if(!strcmp(gcFunction,"tSerologiaTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tSerologia(LANG_NB_CONFIRMNEW);
			}
			else
				tSerologia("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tSerologia Aux Panel",100);
	switch(guMode)
        {
                case 2000:
		...
		default:
			...
			printf("Tabla de Serologia<br>(RADBuildInfo %.32s)\n",sgcBuildInfo);
...
void ExttSerologiaGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uSerologia"))
		{
			sscanf(gentries[i].val,"%u",&uSerologia);
			guMode=6;
		}
	}
	tSerologia("");
...
void ExttSerologiaSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tSerologia ORDER BY"
				" uSerologia",
				VAR_LIST_tSerologia);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tSerologia,tClient WHERE tSerologia.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uSerologia",
					VAR_LIST_tSerologia,uContactParentCompany,uContactParentCompany);
...
void ExttSerologiaSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tSerologia WHERE uSerologia=%u",
			VAR_LIST_tSerologia,uSerologia);
	else
                sprintf(gcQuery,"SELECT %s FROM tSerologia,tClient"
                                " WHERE tSerologia.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tSerologia.uSerologia=%u",
                        		VAR_LIST_tSerologia
...
void ExttSerologiaListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tSerologia",
				VAR_LIST_tSerologia);
	else
		sprintf(gcQuery,"SELECT %s FROM tSerologia,tClient"
				" WHERE tSerologia.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttSerologiaListFilter()
        if(!strcmp(gcFilter,"uSerologia"))
        {
                sscanf(gcCommand,"%u",&uSerologia);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tSerologia.uSerologia=%u"
						" ORDER BY uSerologia",
						uSerologia);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uSerologia");
        }
...
void ExttSerologiaListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uSerologia"))
                printf("<option>uSerologia</option>");
        else
                printf("<option selected>uSerologia</option>");
...
void tSerologiaNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uSerologia,cLabel FROM tSerologia ORDER BY cLabel");
	else
        ...
        {
        	printf("<p><u>tSerologiaNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tSerologia"
				"&uSerologia=%s>%s</a><br>\n",
				field[0],field[1]);
	}
