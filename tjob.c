/*
FILE
	tJob source code of unxsSalud.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetJob();
				unxsSalud("New tJob table created");
                	}
			else
			{
...
void DeletetJob(void)
{
	...
	{
		unxsSaludLog(uJob,"tJob","Del");
		tJob(LANG_NBR_RECDELETED);
	}
	else
	{
		unxsSaludLog(uJob,"tJob","DelError");
