...
void tMicrobiologiaNavList(void);
	...
	if(!strcmp(gcFunction,"tMicrobiologiaTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tMicrobiologia(LANG_NB_CONFIRMNEW);
			}
			else
				tMicrobiologia("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
	                        guMode=2001;
				tMicrobiologia(LANG_NB_CONFIRMDEL);
			}
			else
				tMicrobiologia("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
				guMode=2002;
				tMicrobiologia(LANG_NB_CONFIRMMOD);
			}
			else
				tMicrobiologia("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tMicrobiologia Aux Panel",100);
	switch(guMode)
        {
                case 2000:
...
void ExttMicrobiologiaGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uMicrobiologia"))
		{
			sscanf(gentries[i].val,"%u",&uMicrobiologia);
			guMode=6;
		}
	}
	tMicrobiologia("");
...
void ExttMicrobiologiaSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tMicrobiologia ORDER BY"
				" uMicrobiologia",
				VAR_LIST_tMicrobiologia);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tMicrobiologia,tClient WHERE tMicrobiologia.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uMicrobiologia",
					VAR_LIST_tMicrobiologia,uContactParentCompany,uContactParentCompany);
...
void ExttMicrobiologiaSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tMicrobiologia WHERE uMicrobiologia=%u",
			VAR_LIST_tMicrobiologia,uMicrobiologia);
	else
                sprintf(gcQuery,"SELECT %s FROM tMicrobiologia,tClient"
                                " WHERE tMicrobiologia.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tMicrobiologia.uMicrobiologia=%u",
                        		VAR_LIST_tMicrobiologia
...
void ExttMicrobiologiaListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tMicrobiologia",
				VAR_LIST_tMicrobiologia);
	else
		sprintf(gcQuery,"SELECT %s FROM tMicrobiologia,tClient"
				" WHERE tMicrobiologia.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttMicrobiologiaListFilter()
        if(!strcmp(gcFilter,"uMicrobiologia"))
        {
                sscanf(gcCommand,"%u",&uMicrobiologia);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tMicrobiologia.uMicrobiologia=%u"
						" ORDER BY uMicrobiologia",
						uMicrobiologia);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uMicrobiologia");
        }
...
void ExttMicrobiologiaListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uMicrobiologia"))
                printf("<option>uMicrobiologia</option>");
        else
                printf("<option selected>uMicrobiologia</option>");
...
void tMicrobiologiaNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uMicrobiologia,cLabel FROM tMicrobiologia ORDER BY cLabel");
	else
		sprintf(gcQuery,"SELECT tMicrobiologia.uMicrobiologia,"
				" tMicrobiologia.cLabel"
        ...
        {
        	printf("<p><u>tMicrobiologiaNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tMicrobiologia"
				"&uMicrobiologia=%s>%s</a><br>\n",
				field[0],field[1]);
	}
