...
FILE 
	...
	unxsRAD created application file for unxsSalud.cgi
PURPOSE
...
static char *sgcBuildInfo="39e92f3-dirty";
...
int main(int iArgc, char *cArgv[])
{
	...
	if(!strstr(cArgv[0],"unxsSalud.cgi"))
	...
	if(strcmp(getenv("REQUEST_METHOD"),"POST"))
	{
		...
		if(gcFunction[0])
		{
			...
			{
				printf("Set-Cookie: unxsSaludLogin=; discard; expires=\"Mon, 01-Jan-1971 00:10:10 GMT\"\n");
				printf("Set-Cookie: unxsSaludPasswd=; discard; expires=\"Mon, 01-Jan-1971 00:10:10 GMT\"\n");
				printf("Set-Cookie: unxsSaludSessionCookie=; discard; expires=\"Mon, 01-Jan-1971 00:10:10 GMT\"\n");
			...
			//funcMainGetMenu()
			if(!strcmp(gcFunction,"tPaciente"))
				ExttPacienteGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tConsulta"))
				ExttConsultaGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tMotivo"))
				ExttMotivoGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tCentro"))
				ExttCentroGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tAntecedente"))
				ExttAntecedenteGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tInternacion"))
				ExttInternacionGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tMedico"))
				ExttMedicoGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tMedicacion"))
				ExttMedicacionGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tInmunizacion"))
				ExttInmunizacionGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tTipoConsulta"))
				ExttTipoConsultaGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tLaboratorio"))
				ExttLaboratorioGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tSangre"))
				ExttSangreGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tSerologia"))
				ExttSerologiaGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tMicrobiologia"))
				ExttMicrobiologiaGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tHemocultivo"))
				ExttHemocultivoGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tUrocultivo"))
				ExttUrocultivoGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tCultivoCateter"))
				ExttCultivoCateterGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tCultivoPB"))
				ExttCultivoPBGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tCultivoHueso"))
				ExttCultivoHuesoGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tCultivoEsputo"))
				ExttCultivoEsputoGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tCultivoLCR"))
				ExttCultivoLCRGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tOrina"))
				ExttOrinaGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tVirologia"))
				ExttVirologiaGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tVitales"))
				ExttVitalesGetHook(gentries,x);
	...
	tPacienteCommands(entries,x);
	tConsultaCommands(entries,x);
	tMotivoCommands(entries,x);
	tCentroCommands(entries,x);
	tAntecedenteCommands(entries,x);
	tInternacionCommands(entries,x);
	tMedicoCommands(entries,x);
	tMedicacionCommands(entries,x);
	tInmunizacionCommands(entries,x);
	tTipoConsultaCommands(entries,x);
	tLaboratorioCommands(entries,x);
	tSangreCommands(entries,x);
	tSerologiaCommands(entries,x);
	tMicrobiologiaCommands(entries,x);
	tHemocultivoCommands(entries,x);
	tUrocultivoCommands(entries,x);
	tCultivoCateterCommands(entries,x);
	tCultivoPBCommands(entries,x);
	tCultivoHuesoCommands(entries,x);
	tCultivoEsputoCommands(entries,x);
	tCultivoLCRCommands(entries,x);
	tOrinaCommands(entries,x);
	tVirologiaCommands(entries,x);
	tVitalesCommands(entries,x);
...
void HeaderRAD4(char *cTitle, int iJs)
{
	...
	printf("</head><body><form name=formMain action=unxsSalud.cgi method=post><blockquote>\n");
	...
	//funcMainNavBars()
	if(!strcmp(gcFunction,"tPaciente") || !strcmp(gcFunction,"tPacienteTools") ||
			!strcmp(gcFunction,"tPacienteList"))
		ExttPacienteNavBar();
	else if(!strcmp(gcFunction,"tConsulta") || !strcmp(gcFunction,"tConsultaTools") ||
			!strcmp(gcFunction,"tConsultaList"))
		ExttConsultaNavBar();
	else if(!strcmp(gcFunction,"tMotivo") || !strcmp(gcFunction,"tMotivoTools") ||
			!strcmp(gcFunction,"tMotivoList"))
		ExttMotivoNavBar();
	else if(!strcmp(gcFunction,"tCentro") || !strcmp(gcFunction,"tCentroTools") ||
			!strcmp(gcFunction,"tCentroList"))
		ExttCentroNavBar();
	else if(!strcmp(gcFunction,"tAntecedente") || !strcmp(gcFunction,"tAntecedenteTools") ||
			!strcmp(gcFunction,"tAntecedenteList"))
		ExttAntecedenteNavBar();
	else if(!strcmp(gcFunction,"tInternacion") || !strcmp(gcFunction,"tInternacionTools") ||
			!strcmp(gcFunction,"tInternacionList"))
		ExttInternacionNavBar();
	else if(!strcmp(gcFunction,"tMedico") || !strcmp(gcFunction,"tMedicoTools") ||
			!strcmp(gcFunction,"tMedicoList"))
		ExttMedicoNavBar();
	else if(!strcmp(gcFunction,"tMedicacion") || !strcmp(gcFunction,"tMedicacionTools") ||
			!strcmp(gcFunction,"tMedicacionList"))
		ExttMedicacionNavBar();
	else if(!strcmp(gcFunction,"tInmunizacion") || !strcmp(gcFunction,"tInmunizacionTools") ||
			!strcmp(gcFunction,"tInmunizacionList"))
		ExttInmunizacionNavBar();
	else if(!strcmp(gcFunction,"tTipoConsulta") || !strcmp(gcFunction,"tTipoConsultaTools") ||
			!strcmp(gcFunction,"tTipoConsultaList"))
		ExttTipoConsultaNavBar();
	else if(!strcmp(gcFunction,"tLaboratorio") || !strcmp(gcFunction,"tLaboratorioTools") ||
			!strcmp(gcFunction,"tLaboratorioList"))
		ExttLaboratorioNavBar();
	else if(!strcmp(gcFunction,"tSangre") || !strcmp(gcFunction,"tSangreTools") ||
			!strcmp(gcFunction,"tSangreList"))
		ExttSangreNavBar();
	else if(!strcmp(gcFunction,"tSerologia") || !strcmp(gcFunction,"tSerologiaTools") ||
			!strcmp(gcFunction,"tSerologiaList"))
		ExttSerologiaNavBar();
	else if(!strcmp(gcFunction,"tMicrobiologia") || !strcmp(gcFunction,"tMicrobiologiaTools") ||
			!strcmp(gcFunction,"tMicrobiologiaList"))
		ExttMicrobiologiaNavBar();
	else if(!strcmp(gcFunction,"tHemocultivo") || !strcmp(gcFunction,"tHemocultivoTools") ||
			!strcmp(gcFunction,"tHemocultivoList"))
		ExttHemocultivoNavBar();
	else if(!strcmp(gcFunction,"tUrocultivo") || !strcmp(gcFunction,"tUrocultivoTools") ||
			!strcmp(gcFunction,"tUrocultivoList"))
		ExttUrocultivoNavBar();
	else if(!strcmp(gcFunction,"tCultivoCateter") || !strcmp(gcFunction,"tCultivoCateterTools") ||
			!strcmp(gcFunction,"tCultivoCateterList"))
		ExttCultivoCateterNavBar();
	else if(!strcmp(gcFunction,"tCultivoPB") || !strcmp(gcFunction,"tCultivoPBTools") ||
			!strcmp(gcFunction,"tCultivoPBList"))
		ExttCultivoPBNavBar();
	else if(!strcmp(gcFunction,"tCultivoHueso") || !strcmp(gcFunction,"tCultivoHuesoTools") ||
			!strcmp(gcFunction,"tCultivoHuesoList"))
		ExttCultivoHuesoNavBar();
	else if(!strcmp(gcFunction,"tCultivoEsputo") || !strcmp(gcFunction,"tCultivoEsputoTools") ||
			!strcmp(gcFunction,"tCultivoEsputoList"))
		ExttCultivoEsputoNavBar();
	else if(!strcmp(gcFunction,"tCultivoLCR") || !strcmp(gcFunction,"tCultivoLCRTools") ||
			!strcmp(gcFunction,"tCultivoLCRList"))
		ExttCultivoLCRNavBar();
	else if(!strcmp(gcFunction,"tOrina") || !strcmp(gcFunction,"tOrinaTools") ||
			!strcmp(gcFunction,"tOrinaList"))
		ExttOrinaNavBar();
	else if(!strcmp(gcFunction,"tVirologia") || !strcmp(gcFunction,"tVirologiaTools") ||
			!strcmp(gcFunction,"tVirologiaList"))
		ExttVirologiaNavBar();
	else if(!strcmp(gcFunction,"tVitales") || !strcmp(gcFunction,"tVitalesTools") ||
			!strcmp(gcFunction,"tVitalesList"))
		ExttVitalesNavBar();
	...
	//Login info
	printf("<font size=3><b>unxsSalud</b></font> \n ");
	printf("&nbsp;&nbsp;&nbsp;<font color=red>%s logged in from %s [%s]</font>",
							gcUser,gcHost,cUserLevel(guPermLevel));

	//Logout link
	if(guSSLCookieLogin)
		printf(" <a title='Erase login cookies' href=unxsSalud.cgi?gcFunction=Logout>Logout</a> ");
	...
	if(strncmp(gcFunction,"Main",4))
		...
		printf(" id=current>\n");
		printf("\t\t\t<a title='Home start page' href=unxsSalud.cgi?gcFunction=Main>Main</a>\n");
	printf("\t\t\t</li>\n");
	...
	if(guPermLevel>=7)
	{
	  ...
	  if(strcmp(gcFunction,"tPaciente") && strcmp(gcFunction,"tPacienteTools") &&
		  ...
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Pacientes' href=unxsSalud.cgi?gcFunction=tPaciente>tPaciente</a>\n");
	}
	//tConsulta
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tConsulta") && strcmp(gcFunction,"tConsultaTools") &&
			strcmp(gcFunction,"tConsultaList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Registro de consulta' href=unxsSalud.cgi?gcFunction=tConsulta>tConsulta</a>\n");
	}
	//tMotivo
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tMotivo") && strcmp(gcFunction,"tMotivoTools") &&
			strcmp(gcFunction,"tMotivoList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Motivo de consulta' href=unxsSalud.cgi?gcFunction=tMotivo>tMotivo</a>\n");
	}
	//tCentro
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tCentro") && strcmp(gcFunction,"tCentroTools") &&
			strcmp(gcFunction,"tCentroList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Centro medico' href=unxsSalud.cgi?gcFunction=tCentro>tCentro</a>\n");
	}
	//tAntecedente
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tAntecedente") && strcmp(gcFunction,"tAntecedenteTools") &&
			strcmp(gcFunction,"tAntecedenteList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Antecedentes personales' href=unxsSalud.cgi?gcFunction=tAntecedente>tAntecedente</a>\n");
	}
	//tInternacion
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tInternacion") && strcmp(gcFunction,"tInternacionTools") &&
			strcmp(gcFunction,"tInternacionList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Historia de internaciones' href=unxsSalud.cgi?gcFunction=tInternacion>tInternacion</a>\n");
	}
	//tMedico
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tMedico") && strcmp(gcFunction,"tMedicoTools") &&
			strcmp(gcFunction,"tMedicoList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Registro de medicos' href=unxsSalud.cgi?gcFunction=tMedico>tMedico</a>\n");
	}
	//tMedicacion
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tMedicacion") && strcmp(gcFunction,"tMedicacionTools") &&
			strcmp(gcFunction,"tMedicacionList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Medicación Habitual' href=unxsSalud.cgi?gcFunction=tMedicacion>tMedicacion</a>\n");
	}
	//tInmunizacion
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tInmunizacion") && strcmp(gcFunction,"tInmunizacionTools") &&
			strcmp(gcFunction,"tInmunizacionList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Registro de vacunas' href=unxsSalud.cgi?gcFunction=tInmunizacion>tInmunizacion</a>\n");
	}
	//tTipoConsulta
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tTipoConsulta") && strcmp(gcFunction,"tTipoConsultaTools") &&
			strcmp(gcFunction,"tTipoConsultaList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Tipo de consulta' href=unxsSalud.cgi?gcFunction=tTipoConsulta>tTipoConsulta</a>\n");
	}
	//tLaboratorio
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tLaboratorio") && strcmp(gcFunction,"tLaboratorioTools") &&
			strcmp(gcFunction,"tLaboratorioList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Tabla de valores de Laboratorio' href=unxsSalud.cgi?gcFunction=tLaboratorio>tLaboratorio</a>\n");
	}
	//tSangre
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tSangre") && strcmp(gcFunction,"tSangreTools") &&
			strcmp(gcFunction,"tSangreList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Tabla de valores de laboratorio de sangre' href=unxsSalud.cgi?gcFunction=tSangre>tSangre</a>\n");
	}
	//tSerologia
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tSerologia") && strcmp(gcFunction,"tSerologiaTools") &&
			strcmp(gcFunction,"tSerologiaList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Tabla de valores de laboratorio de Serologia' href=unxsSalud.cgi?gcFunction=tSerologia>tSerologia</a>\n");
	}
	//tMicrobiologia
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tMicrobiologia") && strcmp(gcFunction,"tMicrobiologiaTools") &&
			strcmp(gcFunction,"tMicrobiologiaList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Tabla de tablas de valores de laboratorios de microbiologia' href=unxsSalud.cgi?gcFunction=tMicrobiologia>tMicrobiologia</a>\n");
	}
	//tHemocultivo
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tHemocultivo") && strcmp(gcFunction,"tHemocultivoTools") &&
			strcmp(gcFunction,"tHemocultivoList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Tabla de tablas de valores de laboratorio Hemocultivo' href=unxsSalud.cgi?gcFunction=tHemocultivo>tHemocultivo</a>\n");
	}
	//tUrocultivo
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tUrocultivo") && strcmp(gcFunction,"tUrocultivoTools") &&
			strcmp(gcFunction,"tUrocultivoList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Tabla de tablas de valores de laboratorio Urocultivo' href=unxsSalud.cgi?gcFunction=tUrocultivo>tUrocultivo</a>\n");
	}
	//tCultivoCateter
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tCultivoCateter") && strcmp(gcFunction,"tCultivoCateterTools") &&
			strcmp(gcFunction,"tCultivoCateterList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Tabla de tablas de valores de laboratorio Cateter' href=unxsSalud.cgi?gcFunction=tCultivoCateter>tCultivoCateter</a>\n");
	}
	//tCultivoPB
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tCultivoPB") && strcmp(gcFunction,"tCultivoPBTools") &&
			strcmp(gcFunction,"tCultivoPBList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Tabla de tablas de valores de laboratorio PB' href=unxsSalud.cgi?gcFunction=tCultivoPB>tCultivoPB</a>\n");
	}
	//tCultivoHueso
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tCultivoHueso") && strcmp(gcFunction,"tCultivoHuesoTools") &&
			strcmp(gcFunction,"tCultivoHuesoList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Tabla de tablas de valores de laboratorio Hueso' href=unxsSalud.cgi?gcFunction=tCultivoHueso>tCultivoHueso</a>\n");
	}
	//tCultivoEsputo
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tCultivoEsputo") && strcmp(gcFunction,"tCultivoEsputoTools") &&
			strcmp(gcFunction,"tCultivoEsputoList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Tabla de tablas de valores de laboratorio Esputo' href=unxsSalud.cgi?gcFunction=tCultivoEsputo>tCultivoEsputo</a>\n");
	}
	//tCultivoLCR
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tCultivoLCR") && strcmp(gcFunction,"tCultivoLCRTools") &&
			strcmp(gcFunction,"tCultivoLCRList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Tabla de tablas de valores de laboratorio LCR' href=unxsSalud.cgi?gcFunction=tCultivoLCR>tCultivoLCR</a>\n");
	}
	//tOrina
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tOrina") && strcmp(gcFunction,"tOrinaTools") &&
			strcmp(gcFunction,"tOrinaList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Tabla de valores de laboratorio de orina' href=unxsSalud.cgi?gcFunction=tOrina>tOrina</a>\n");
	}
	//tVirologia
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tVirologia") && strcmp(gcFunction,"tVirologiaTools") &&
			strcmp(gcFunction,"tVirologiaList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Tabla de cargas virales' href=unxsSalud.cgi?gcFunction=tVirologia>tVirologia</a>\n");
	}
	//tVitales
	if(guPermLevel>=20)
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tVitales") && strcmp(gcFunction,"tVitalesTools") &&
			strcmp(gcFunction,"tVitalesList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Tabla de signos vitales' href=unxsSalud.cgi?gcFunction=tVitales>tVitales</a>\n");
	}
	//tClient
	if(guPermLevel>=20)
	{
	  ...
	  if(strcmp(gcFunction,"tClient") && strcmp(gcFunction,"tClientTools") &&
		  ...
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Organizations and their contacts' href=unxsSalud.cgi?gcFunction=tClient>tClient</a>\n");
	}
	//tAuthorize
	if(guPermLevel>=20)
	{
	  ...
	  if(strcmp(gcFunction,"tAuthorize") && strcmp(gcFunction,"tAuthorizeTools") &&
		  ...
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Contact credentials' href=unxsSalud.cgi?gcFunction=tAuthorize>tAuthorize</a>\n");
	}
	//tServer
	if(guPermLevel>=20)
	{
	  ...
	  if(strcmp(gcFunction,"tServer") && strcmp(gcFunction,"tServerTools") &&
		  ...
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Servers' href=unxsSalud.cgi?gcFunction=tServer>tServer</a>\n");
	}
	//tConfiguration
	if(guPermLevel>=20)
	{
	  ...
	  if(strcmp(gcFunction,"tConfiguration") && strcmp(gcFunction,"tConfigurationTools") &&
		  ...
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='General configuration' href=unxsSalud.cgi?gcFunction=tConfiguration>tConfiguration</a>\n");
	}
	//tStatus
	if(guPermLevel>=20)
	{
	  ...
	  if(strcmp(gcFunction,"tStatus") && strcmp(gcFunction,"tStatusTools") &&
		  ...
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='General status labels' href=unxsSalud.cgi?gcFunction=tStatus>tStatus</a>\n");
	}
	//tJobStatus
	if(guPermLevel>=20)
	{
	  ...
	  if(strcmp(gcFunction,"tJobStatus") && strcmp(gcFunction,"tJobStatusTools") &&
		  ...
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Job queue status labels' href=unxsSalud.cgi?gcFunction=tJobStatus>tJobStatus</a>\n");
	}
	//tJob
	if(guPermLevel>=20)
	{
	  ...
	  if(strcmp(gcFunction,"tJob") && strcmp(gcFunction,"tJobTools") &&
		  ...
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Job queue' href=unxsSalud.cgi?gcFunction=tJob>tJob</a>\n");
	}
	//tGlossary
	if(guPermLevel>=20)
	{
	  ...
	  if(strcmp(gcFunction,"tGlossary") && strcmp(gcFunction,"tGlossaryTools") &&
		  ...
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Glossary and help' href=unxsSalud.cgi?gcFunction=tGlossary>tGlossary</a>\n");
	}
	//tTemplate
	if(guPermLevel>=20)
	{
	  ...
	  if(strcmp(gcFunction,"tTemplate") && strcmp(gcFunction,"tTemplateTools") &&
		  ...
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Templates' href=unxsSalud.cgi?gcFunction=tTemplate>tTemplate</a>\n");
	}
	//tTemplateSet
	if(guPermLevel>=20)
	{
	  ...
	  if(strcmp(gcFunction,"tTemplateSet") && strcmp(gcFunction,"tTemplateSetTools") &&
		  ...
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Template sets' href=unxsSalud.cgi?gcFunction=tTemplateSet>tTemplateSet</a>\n");
	}
	//tTemplateType
	if(guPermLevel>=20)
	{
	  ...
	  if(strcmp(gcFunction,"tTemplateType") && strcmp(gcFunction,"tTemplateTypeTools") &&
		  ...
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Template types' href=unxsSalud.cgi?gcFunction=tTemplateType>tTemplateType</a>\n");
	}
	//tLog
	if(guPermLevel>=20)
	{
	  ...
	  if(strcmp(gcFunction,"tLog") && strcmp(gcFunction,"tLogTools") &&
		  ...
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Log' href=unxsSalud.cgi?gcFunction=tLog>tLog</a>\n");
	}
	//tLogType
	if(guPermLevel>=20)
	{
	  ...
	  if(strcmp(gcFunction,"tLogType") && strcmp(gcFunction,"tLogTypeTools") &&
		  ...
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Log type' href=unxsSalud.cgi?gcFunction=tLogType>tLogType</a>\n");
	}
	//tLogMonth
	if(guPermLevel>=20)
	{
	  ...
	  if(strcmp(gcFunction,"tLogMonth") && strcmp(gcFunction,"tLogMonthTools") &&
		  ...
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Archive of logs' href=unxsSalud.cgi?gcFunction=tLogMonth>tLogMonth</a>\n");
	}
	//tMonth
	if(guPermLevel>=20)
	{
	  ...
	  if(strcmp(gcFunction,"tMonth") && strcmp(gcFunction,"tMonthTools") &&
		  ...
		  printf(" id=current>\n");
	  printf("\t\t\t<a title='Month of year' href=unxsSalud.cgi?gcFunction=tMonth>tMonth</a>\n");
	}
...
char *TextAreaSave(char *cField)
{
	...
	if(!cCopy) unxsSalud("TextAreaInput() malloc error");
...
char *TransformAngleBrackets(char *cField)
{
	...
	if(!cCopy) unxsSalud("TransformAngleBrackets() malloc error");
...
char *EncodeDoubleQuotes(char *cField)
{
	...
	if(!cCopy) unxsSalud("EncodeDoubleQuotes() malloc error");
...
void InitialInstall(void)
{
	unxsSalud("Please run\
		unxsSalud.cgi Initialize &lt;mysqlpwd&gt; from the command line");
...
void SetLogin(void)
{
	if( iValidLogin(0) )
	{
		printf("Set-Cookie: unxsSaludLogin=%s; secure; httponly;\n",gcLogin);
		printf("Set-Cookie: unxsSaludPasswd=%s; secure; httponly;\n",gcPasswd);
...
void SSLCookieLogin(void)
{
	...
	if(getenv("HTTPS")==NULL) 
		unxsSalud("Non SSL access denied");
	...
	if((ptr=strstr(gcCookie,"unxsSaludLogin=")))
	{
		ptr+=strlen("unxsSaludLogin=");
		if((ptr2=strchr(ptr,';')))
		{
			*ptr2=0;
	...
	if((ptr=strstr(gcCookie,"unxsSaludPasswd=")))
	{
		ptr+=strlen("unxsSaludPasswd=");
		if((ptr2=strchr(ptr,';')))
		{
			*ptr2=0;
...
void htmlPlainTextError(const char *cError)
{
	...
	printf("Please report this unxsSalud fatal error ASAP:\n%s\n",cError);
...
void SetSessionCookie(void)
{
	printf("Set-Cookie: unxsSaludSessionCookie=uProject=%u|uTable=%u|uField=%u|; Secure; HttpOnly\n",
...
void GetSessionCookie(void)
{
	char *ptr;
	char *ptr2;
	char cunxsSaludSessionCookie[512]={""};

	if(getenv("HTTP_COOKIE")!=NULL)
		sprintf(gcCookie,"%.1022s",getenv("HTTP_COOKIE"));
	if(gcCookie[0])
	{
		if((ptr=strstr(gcCookie,"unxsSaludSessionCookie=")))
		{
			ptr+=strlen("unxsSaludSessionCookie=");
			if((ptr2=strchr(ptr,';')))
			{
				*ptr2=0;
				sprintf(cunxsSaludSessionCookie,"%.511s",ptr);
			}
			else
				 sprintf(cunxsSaludSessionCookie,"%.511s",ptr);
	...
	if(cunxsSaludSessionCookie[0])
	{
		if((ptr=strstr(cunxsSaludSessionCookie,"uProject=")))
		{
			ptr+=strlen("uProject=");
			if((ptr2=strchr(ptr,'|')))
			{
		...
		if((ptr=strstr(cunxsSaludSessionCookie,"uTable=")))
		{
			ptr+=strlen("uTable=");
			if((ptr2=strchr(ptr,'|')))
			{
		...
		if((ptr=strstr(cunxsSaludSessionCookie,"uField=")))
		{
			ptr+=strlen("uField=");
			if((ptr2=strchr(ptr,'|')))
			{
