	...
	if(!strcmp(gcFunction,"tOrinaTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tOrina(LANG_NB_CONFIRMNEW);
			}
			else
				tOrina("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tOrina Aux Panel",100);
	switch(guMode)
        {
                case 2000:
		...
		default:
			...
			printf("Tabla de Orina<br>(RADBuildInfo %.32s)\n",sgcBuildInfo);
...
void ExttOrinaGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uOrina"))
		{
			sscanf(gentries[i].val,"%u",&uOrina);
			guMode=6;
		}
	}
	tOrina("");
...
void ExttOrinaSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tOrina ORDER BY"
				" uOrina",
				VAR_LIST_tOrina);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tOrina,tClient WHERE tOrina.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uOrina",
					VAR_LIST_tOrina,uContactParentCompany,uContactParentCompany);
...
void ExttOrinaSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tOrina WHERE uOrina=%u",
			VAR_LIST_tOrina,uOrina);
	else
                sprintf(gcQuery,"SELECT %s FROM tOrina,tClient"
                                " WHERE tOrina.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tOrina.uOrina=%u",
                        		VAR_LIST_tOrina
...
void ExttOrinaListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tOrina",
				VAR_LIST_tOrina);
	else
		sprintf(gcQuery,"SELECT %s FROM tOrina,tClient"
				" WHERE tOrina.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttOrinaListFilter()
        if(!strcmp(gcFilter,"uOrina"))
        {
                sscanf(gcCommand,"%u",&uOrina);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tOrina.uOrina=%u"
						" ORDER BY uOrina",
						uOrina);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uOrina");
        }
...
void ExttOrinaListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uOrina"))
                printf("<option>uOrina</option>");
        else
                printf("<option selected>uOrina</option>");
...
void tOrinaNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uOrina,cLabel FROM tOrina ORDER BY cLabel");
	else
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tOrina"
				"&uOrina=%s>%s</a><br>\n",
				field[0],field[1]);
	}
