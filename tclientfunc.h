	...
	switch(guMode)
        {
			...
			printf("<u>Authorize: Step 1 Tips</u><br>Depending on the user level you may authorize a contact to access "
				...
				"'Back-Office Root' that will allow the user full access to this back-office unxsSalud.cgi interface.<p>\n");
			if(guPermLevel>7)
...
void ContactsNavList(void)
{
	...
	if(uOwner!=1)
	{
		...
		if(mysql_num_rows(res))
		{
        		printf("<p><u>tAuthorize LoginNavList</u><br>\n");
			while((field=mysql_fetch_row(res)))
				printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tAuthorize&uAuthorize=%s>%s<a><br>",field[1],field[0]);
		}
        	mysql_free_result(res);
		printf("\n");
	}
	...
	if(mysql_num_rows(res))
	{
		...
		while((field=mysql_fetch_row(res)))
        	{
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tClient&uClient=%s&uOnlyOrgs=%u",field[0],uOnlyOrgs);
			if(cSearch[0])
			{
				spacetoplus(cSearch);
...
void htmlRecordContext(void)
{
	...
	if(uOwner>1)
		printf("'%s' appears to be a contact of <a class=darkLink href=unxsSalud.cgi?gcFunction=tClient&uClient=%u>'%s'</a>",cLabel,uOwner,ForeignKey("tClient","cLabel",uOwner));
