/*
FILE
	tMonth source code of unxsSalud.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetMonth();
				unxsSalud("New tMonth table created");
                	}
			else
			{
...
void NewtMonth(unsigned uMode)
{
	...
	unxsSaludLog(uMonth,"tMonth","New");
#endif
...
void DeletetMonth(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uMonth,"tMonth","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uMonth,"tMonth","DelError");
#endif
...
void ModtMonth(void)
{
	...
	unxsSaludLog(uMonth,"tMonth","Mod");
#endif
	tMonth(gcQuery);
