...
void tTipoConsultaNavList(void);
	...
	if(!strcmp(gcFunction,"tTipoConsultaTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tTipoConsulta(LANG_NB_CONFIRMNEW);
			}
			else
				tTipoConsulta("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
	                        guMode=2001;
				tTipoConsulta(LANG_NB_CONFIRMDEL);
			}
			else
				tTipoConsulta("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
				guMode=2002;
				tTipoConsulta(LANG_NB_CONFIRMMOD);
			}
			else
				tTipoConsulta("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tTipoConsulta Aux Panel",100);
	switch(guMode)
        {
                case 2000:
...
void ExttTipoConsultaGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uTipoConsulta"))
		{
			sscanf(gentries[i].val,"%u",&uTipoConsulta);
			guMode=6;
		}
	}
	tTipoConsulta("");
...
void ExttTipoConsultaSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tTipoConsulta ORDER BY"
				" uTipoConsulta",
				VAR_LIST_tTipoConsulta);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tTipoConsulta,tClient WHERE tTipoConsulta.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uTipoConsulta",
					VAR_LIST_tTipoConsulta,uContactParentCompany,uContactParentCompany);
...
void ExttTipoConsultaSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tTipoConsulta WHERE uTipoConsulta=%u",
			VAR_LIST_tTipoConsulta,uTipoConsulta);
	else
                sprintf(gcQuery,"SELECT %s FROM tTipoConsulta,tClient"
                                " WHERE tTipoConsulta.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tTipoConsulta.uTipoConsulta=%u",
                        		VAR_LIST_tTipoConsulta
...
void ExttTipoConsultaListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tTipoConsulta",
				VAR_LIST_tTipoConsulta);
	else
		sprintf(gcQuery,"SELECT %s FROM tTipoConsulta,tClient"
				" WHERE tTipoConsulta.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttTipoConsultaListFilter()
        if(!strcmp(gcFilter,"uTipoConsulta"))
        {
                sscanf(gcCommand,"%u",&uTipoConsulta);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tTipoConsulta.uTipoConsulta=%u"
						" ORDER BY uTipoConsulta",
						uTipoConsulta);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uTipoConsulta");
        }
...
void ExttTipoConsultaListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uTipoConsulta"))
                printf("<option>uTipoConsulta</option>");
        else
                printf("<option selected>uTipoConsulta</option>");
...
void tTipoConsultaNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uTipoConsulta,cLabel FROM tTipoConsulta ORDER BY cLabel");
	else
		sprintf(gcQuery,"SELECT tTipoConsulta.uTipoConsulta,"
				" tTipoConsulta.cLabel"
        ...
        {
        	printf("<p><u>tTipoConsultaNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tTipoConsulta"
				"&uTipoConsulta=%s>%s</a><br>\n",
				field[0],field[1]);
	}
