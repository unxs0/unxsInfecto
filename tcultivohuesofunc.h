...
void tCultivoHuesoNavList(void);
	...
	if(!strcmp(gcFunction,"tCultivoHuesoTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tCultivoHueso(LANG_NB_CONFIRMNEW);
			}
			else
				tCultivoHueso("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
	                        guMode=2001;
				tCultivoHueso(LANG_NB_CONFIRMDEL);
			}
			else
				tCultivoHueso("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
				guMode=2002;
				tCultivoHueso(LANG_NB_CONFIRMMOD);
			}
			else
				tCultivoHueso("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tCultivoHueso Aux Panel",100);
	switch(guMode)
        {
                case 2000:
...
void ExttCultivoHuesoGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uCultivoHueso"))
		{
			sscanf(gentries[i].val,"%u",&uCultivoHueso);
			guMode=6;
		}
	}
	tCultivoHueso("");
...
void ExttCultivoHuesoSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tCultivoHueso ORDER BY"
				" uCultivoHueso",
				VAR_LIST_tCultivoHueso);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tCultivoHueso,tClient WHERE tCultivoHueso.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uCultivoHueso",
					VAR_LIST_tCultivoHueso,uContactParentCompany,uContactParentCompany);
...
void ExttCultivoHuesoSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tCultivoHueso WHERE uCultivoHueso=%u",
			VAR_LIST_tCultivoHueso,uCultivoHueso);
	else
                sprintf(gcQuery,"SELECT %s FROM tCultivoHueso,tClient"
                                " WHERE tCultivoHueso.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tCultivoHueso.uCultivoHueso=%u",
                        		VAR_LIST_tCultivoHueso
...
void ExttCultivoHuesoListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tCultivoHueso",
				VAR_LIST_tCultivoHueso);
	else
		sprintf(gcQuery,"SELECT %s FROM tCultivoHueso,tClient"
				" WHERE tCultivoHueso.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttCultivoHuesoListFilter()
        if(!strcmp(gcFilter,"uCultivoHueso"))
        {
                sscanf(gcCommand,"%u",&uCultivoHueso);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tCultivoHueso.uCultivoHueso=%u"
						" ORDER BY uCultivoHueso",
						uCultivoHueso);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uCultivoHueso");
        }
...
void ExttCultivoHuesoListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uCultivoHueso"))
                printf("<option>uCultivoHueso</option>");
        else
                printf("<option selected>uCultivoHueso</option>");
...
void tCultivoHuesoNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uCultivoHueso,cLabel FROM tCultivoHueso ORDER BY cLabel");
	else
		sprintf(gcQuery,"SELECT tCultivoHueso.uCultivoHueso,"
				" tCultivoHueso.cLabel"
        ...
        {
        	printf("<p><u>tCultivoHuesoNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tCultivoHueso"
				"&uCultivoHueso=%s>%s</a><br>\n",
				field[0],field[1]);
	}
