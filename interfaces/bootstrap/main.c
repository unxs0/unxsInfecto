/*
FILE 
	unxsSalud/interfaces/bootstrap/main.c
...
int main(int argc, char *argv[])
{
	...
	if(gcFunction[0])
	{
		if(!strncmp(gcFunction,"Logout",5))
		{
			...
			printf("Set-Cookie: unxsSaludSessionId=; discard; expires=\"Mon, 01-Jan-1971 00:10:10 GMT\"\n");
			printf("Set-Cookie: unxsSaludSessionHash=; discard; expires=\"Mon, 01-Jan-1971 00:10:10 GMT\"\n");
...
void htmlLogin(void)
{
	htmlHeader("unxsSalud","Default.Header");
	if(guRequireOTPLogin)
		htmlLoginPage("unxsSalud","LoginOTP.Body");
	else
		htmlLoginPage("unxsSalud","Login.Body");
...
void SSLCookieLogin(void)
{
	...
	if(gcCookie[0])
	{
		if((cP=strstr(gcCookie,"unxsSaludSessionId=")))
		{
			cP+=strlen("unxsSaludSessionId=");
			if((cP2=strchr(cP,';')))
			{
				*cP2=0;
		...
		if((cP=strstr(gcCookie,"unxsSaludSessionHash=")))
		{
			cP+=strlen("unxsSaludSessionHash=");
			if((cP2=strchr(cP,';')))
			{
				*cP2=0;
...
void SetLogin(void)
{
	if(iValidLogin(0))
	{
		printf("Set-Cookie: unxsSaludSessionId=%s; secure;\n",gcLogin);
		printf("Set-Cookie: unxsSaludSessionHash=%s; secure;\n",gcPasswd);
		sprintf(gcUser,"%.41s",gcLogin);
		GetPLAndClient(gcUser);
		guSSLCookieLogin=1;
