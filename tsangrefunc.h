	...
	if(!strcmp(gcFunction,"tSangreTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tSangre(LANG_NB_CONFIRMNEW);
			}
			else
				tSangre("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tSangre Aux Panel",100);
	switch(guMode)
        {
                case 2000:
		...
		default:
			...
			printf("Tabla de Sangre<br>(RADBuildInfo %.32s)\n",sgcBuildInfo);
...
void ExttSangreGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uSangre"))
		{
			sscanf(gentries[i].val,"%u",&uSangre);
			guMode=6;
		}
	}
	tSangre("");
...
void ExttSangreSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tSangre ORDER BY"
				" uSangre",
				VAR_LIST_tSangre);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tSangre,tClient WHERE tSangre.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uSangre",
					VAR_LIST_tSangre,uContactParentCompany,uContactParentCompany);
...
void ExttSangreSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tSangre WHERE uSangre=%u",
			VAR_LIST_tSangre,uSangre);
	else
                sprintf(gcQuery,"SELECT %s FROM tSangre,tClient"
                                " WHERE tSangre.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tSangre.uSangre=%u",
                        		VAR_LIST_tSangre
...
void ExttSangreListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tSangre",
				VAR_LIST_tSangre);
	else
		sprintf(gcQuery,"SELECT %s FROM tSangre,tClient"
				" WHERE tSangre.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttSangreListFilter()
        if(!strcmp(gcFilter,"uSangre"))
        {
                sscanf(gcCommand,"%u",&uSangre);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tSangre.uSangre=%u"
						" ORDER BY uSangre",
						uSangre);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uSangre");
        }
...
void ExttSangreListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uSangre"))
                printf("<option>uSangre</option>");
        else
                printf("<option selected>uSangre</option>");
...
void tSangreNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uSangre,cLabel FROM tSangre ORDER BY cLabel");
	else
        ...
        {
        	printf("<p><u>tSangreNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tSangre"
				"&uSangre=%s>%s</a><br>\n",
				field[0],field[1]);
	}
