	...
	if(!strcmp(gcFunction,"tMotivoTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tMotivo(LANG_NB_CONFIRMNEW);
			}
			else
				tMotivo("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tMotivo Aux Panel",100);
	switch(guMode)
        {
                case 2000:
		...
		default:
			...
			printf("Tabla de motivos de consultas. Una consulta puede tener 1 o mas motivos de consultas.<br>(RADBuildInfo %.32s)\n",sgcBuildInfo);
...
void ExttMotivoGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uMotivo"))
		{
			sscanf(gentries[i].val,"%u",&uMotivo);
			guMode=6;
		}
	}
	tMotivo("");
...
void ExttMotivoSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tMotivo ORDER BY"
				" uMotivo",
				VAR_LIST_tMotivo);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tMotivo,tClient WHERE tMotivo.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uMotivo",
					VAR_LIST_tMotivo,uContactParentCompany,uContactParentCompany);
...
void ExttMotivoSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tMotivo WHERE uMotivo=%u",
			VAR_LIST_tMotivo,uMotivo);
	else
                sprintf(gcQuery,"SELECT %s FROM tMotivo,tClient"
                                " WHERE tMotivo.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tMotivo.uMotivo=%u",
                        		VAR_LIST_tMotivo
...
void ExttMotivoListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tMotivo",
				VAR_LIST_tMotivo);
	else
		sprintf(gcQuery,"SELECT %s FROM tMotivo,tClient"
				" WHERE tMotivo.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttMotivoListFilter()
        if(!strcmp(gcFilter,"uMotivo"))
        {
                sscanf(gcCommand,"%u",&uMotivo);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tMotivo.uMotivo=%u"
						" ORDER BY uMotivo",
						uMotivo);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uMotivo");
        }
...
void ExttMotivoListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uMotivo"))
                printf("<option>uMotivo</option>");
        else
                printf("<option selected>uMotivo</option>");
...
void tMotivoNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uMotivo,cLabel FROM tMotivo ORDER BY cLabel");
	else
        ...
        {
        	printf("<p><u>tMotivoNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tMotivo"
				"&uMotivo=%s>%s</a><br>\n",
				field[0],field[1]);
	}
