	...
	if(!strcmp(gcFunction,"tVirologiaTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tVirologia(LANG_NB_CONFIRMNEW);
			}
			else
				tVirologia("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tVirologia Aux Panel",100);
	switch(guMode)
        {
                case 2000:
		...
		default:
			...
			printf("Tabla de Virologia<br>(RADBuildInfo %.32s)\n",sgcBuildInfo);
...
void ExttVirologiaGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uVirologia"))
		{
			sscanf(gentries[i].val,"%u",&uVirologia);
			guMode=6;
		}
	}
	tVirologia("");
...
void ExttVirologiaSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tVirologia ORDER BY"
				" uVirologia",
				VAR_LIST_tVirologia);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tVirologia,tClient WHERE tVirologia.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uVirologia",
					VAR_LIST_tVirologia,uContactParentCompany,uContactParentCompany);
...
void ExttVirologiaSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tVirologia WHERE uVirologia=%u",
			VAR_LIST_tVirologia,uVirologia);
	else
                sprintf(gcQuery,"SELECT %s FROM tVirologia,tClient"
                                " WHERE tVirologia.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tVirologia.uVirologia=%u",
                        		VAR_LIST_tVirologia
...
void ExttVirologiaListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tVirologia",
				VAR_LIST_tVirologia);
	else
		sprintf(gcQuery,"SELECT %s FROM tVirologia,tClient"
				" WHERE tVirologia.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttVirologiaListFilter()
        if(!strcmp(gcFilter,"uVirologia"))
        {
                sscanf(gcCommand,"%u",&uVirologia);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tVirologia.uVirologia=%u"
						" ORDER BY uVirologia",
						uVirologia);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uVirologia");
        }
...
void ExttVirologiaListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uVirologia"))
                printf("<option>uVirologia</option>");
        else
                printf("<option selected>uVirologia</option>");
...
void tVirologiaNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uVirologia,cLabel FROM tVirologia ORDER BY cLabel");
	else
        ...
        {
        	printf("<p><u>tVirologiaNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tVirologia"
				"&uVirologia=%s>%s</a><br>\n",
				field[0],field[1]);
	}
