	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetTemplate();
				unxsSalud("New tTemplate table created");
                	}
			else
			{
...
void NewtTemplate(unsigned uMode)
{
	...
	unxsSaludLog(uTemplate,"tTemplate","New");
...
void DeletetTemplate(void)
{
	...
	{
		unxsSaludLog(uTemplate,"tTemplate","Del");
		tTemplate(LANG_NBR_RECDELETED);
	}
	else
	{
		unxsSaludLog(uTemplate,"tTemplate","DelError");
...
void ModtTemplate(void)
{
	...
	unxsSaludLog(uTemplate,"tTemplate","Mod");
	tTemplate(gcQuery);
