/*
FILE
	tTemplateSet source code of unxsSalud.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetTemplateSet();
				unxsSalud("New tTemplateSet table created");
                	}
			else
			{
...
void NewtTemplateSet(unsigned uMode)
{
	...
	unxsSaludLog(uTemplateSet,"tTemplateSet","New");
#endif
...
void DeletetTemplateSet(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uTemplateSet,"tTemplateSet","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uTemplateSet,"tTemplateSet","DelError");
#endif
...
void ModtTemplateSet(void)
{
	...
	unxsSaludLog(uTemplateSet,"tTemplateSet","Mod");
#endif
	tTemplateSet(gcQuery);
