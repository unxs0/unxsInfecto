	...
	if(!strcmp(gcFunction,"tInternacionTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tInternacion(LANG_NB_CONFIRMNEW);
			}
			else
				tInternacion("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
	                        guMode=2001;
				tInternacion(LANG_NB_CONFIRMDEL);
			}
			else
				tInternacion("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
				guMode=2002;
				tInternacion(LANG_NB_CONFIRMMOD);
			}
			else
				tInternacion("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tInternacion Aux Panel",100);
	switch(guMode)
        {
                case 2000:
		...
		default:
			...
			printf("Datos básicos de una internacion. Un paciente puede tener 0 o mas internaciones.<br>(RADBuildInfo %.32s)\n",sgcBuildInfo);
...
void ExttInternacionGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uInternacion"))
		{
			sscanf(gentries[i].val,"%u",&uInternacion);
			guMode=6;
		}
	}
	tInternacion("");
...
void ExttInternacionSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tInternacion ORDER BY"
				" uInternacion",
				VAR_LIST_tInternacion);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tInternacion,tClient WHERE tInternacion.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uInternacion",
					VAR_LIST_tInternacion,uContactParentCompany,uContactParentCompany);
...
void ExttInternacionSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tInternacion WHERE uInternacion=%u",
			VAR_LIST_tInternacion,uInternacion);
	else
                sprintf(gcQuery,"SELECT %s FROM tInternacion,tClient"
                                " WHERE tInternacion.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tInternacion.uInternacion=%u",
                        		VAR_LIST_tInternacion
...
void ExttInternacionListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tInternacion",
				VAR_LIST_tInternacion);
	else
		sprintf(gcQuery,"SELECT %s FROM tInternacion,tClient"
				" WHERE tInternacion.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttInternacionListFilter()
        if(!strcmp(gcFilter,"uInternacion"))
        {
                sscanf(gcCommand,"%u",&uInternacion);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tInternacion.uInternacion=%u"
						" ORDER BY uInternacion",
						uInternacion);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uInternacion");
        }
...
void ExttInternacionListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uInternacion"))
                printf("<option>uInternacion</option>");
        else
                printf("<option selected>uInternacion</option>");
...
void tInternacionNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uInternacion,cLabel FROM tInternacion ORDER BY cLabel");
	else
        ...
        {
        	printf("<p><u>tInternacionNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tInternacion"
				"&uInternacion=%s>%s</a><br>\n",
				field[0],field[1]);
	}
