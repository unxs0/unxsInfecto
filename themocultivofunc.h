	...
	if(!strcmp(gcFunction,"tHemocultivoTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tHemocultivo(LANG_NB_CONFIRMNEW);
			}
			else
				tHemocultivo("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
	                        guMode=2001;
				tHemocultivo(LANG_NB_CONFIRMDEL);
			}
			else
				tHemocultivo("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
				guMode=2002;
				tHemocultivo(LANG_NB_CONFIRMMOD);
			}
			else
				tHemocultivo("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tHemocultivo Aux Panel",100);
	switch(guMode)
        {
                case 2000:
		...
		default:
			...
			printf("Tabla de Hemocultivo<br>(RADBuildInfo %.32s)\n",sgcBuildInfo);
...
void ExttHemocultivoGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uHemocultivo"))
		{
			sscanf(gentries[i].val,"%u",&uHemocultivo);
			guMode=6;
		}
	}
	tHemocultivo("");
...
void ExttHemocultivoSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tHemocultivo ORDER BY"
				" uHemocultivo",
				VAR_LIST_tHemocultivo);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tHemocultivo,tClient WHERE tHemocultivo.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uHemocultivo",
					VAR_LIST_tHemocultivo,uContactParentCompany,uContactParentCompany);
...
void ExttHemocultivoSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tHemocultivo WHERE uHemocultivo=%u",
			VAR_LIST_tHemocultivo,uHemocultivo);
	else
                sprintf(gcQuery,"SELECT %s FROM tHemocultivo,tClient"
                                " WHERE tHemocultivo.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tHemocultivo.uHemocultivo=%u",
                        		VAR_LIST_tHemocultivo
...
void ExttHemocultivoListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tHemocultivo",
				VAR_LIST_tHemocultivo);
	else
		sprintf(gcQuery,"SELECT %s FROM tHemocultivo,tClient"
				" WHERE tHemocultivo.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttHemocultivoListFilter()
        if(!strcmp(gcFilter,"uHemocultivo"))
        {
                sscanf(gcCommand,"%u",&uHemocultivo);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tHemocultivo.uHemocultivo=%u"
						" ORDER BY uHemocultivo",
						uHemocultivo);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uHemocultivo");
        }
...
void ExttHemocultivoListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uHemocultivo"))
                printf("<option>uHemocultivo</option>");
        else
                printf("<option selected>uHemocultivo</option>");
...
void tHemocultivoNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uHemocultivo,cLabel FROM tHemocultivo ORDER BY cLabel");
	else
        ...
        {
        	printf("<p><u>tHemocultivoNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tHemocultivo"
				"&uHemocultivo=%s>%s</a><br>\n",
				field[0],field[1]);
	}
