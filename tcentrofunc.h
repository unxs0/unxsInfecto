	...
	if(!strcmp(gcFunction,"tCentroTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tCentro(LANG_NB_CONFIRMNEW);
			}
			else
				tCentro("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tCentro Aux Panel",100);
	switch(guMode)
        {
                case 2000:
		...
		default:
			...
			printf("Taba de centros medicos<br>(RADBuildInfo %.32s)\n",sgcBuildInfo);
...
void ExttCentroGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uCentro"))
		{
			sscanf(gentries[i].val,"%u",&uCentro);
			guMode=6;
		}
	}
	tCentro("");
...
void ExttCentroSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tCentro ORDER BY"
				" uCentro",
				VAR_LIST_tCentro);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tCentro,tClient WHERE tCentro.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uCentro",
					VAR_LIST_tCentro,uContactParentCompany,uContactParentCompany);
...
void ExttCentroSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tCentro WHERE uCentro=%u",
			VAR_LIST_tCentro,uCentro);
	else
                sprintf(gcQuery,"SELECT %s FROM tCentro,tClient"
                                " WHERE tCentro.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tCentro.uCentro=%u",
                        		VAR_LIST_tCentro
...
void ExttCentroListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tCentro",
				VAR_LIST_tCentro);
	else
		sprintf(gcQuery,"SELECT %s FROM tCentro,tClient"
				" WHERE tCentro.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttCentroListFilter()
        if(!strcmp(gcFilter,"uCentro"))
        {
                sscanf(gcCommand,"%u",&uCentro);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tCentro.uCentro=%u"
						" ORDER BY uCentro",
						uCentro);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uCentro");
        }
...
void ExttCentroListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uCentro"))
                printf("<option>uCentro</option>");
        else
                printf("<option selected>uCentro</option>");
...
void tCentroNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uCentro,cLabel FROM tCentro ORDER BY cLabel");
	else
        ...
        {
        	printf("<p><u>tCentroNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tCentro"
				"&uCentro=%s>%s</a><br>\n",
				field[0],field[1]);
	}
