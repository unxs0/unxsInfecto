/*
FILE
	tGlossary source code of unxsSalud.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetGlossary();
				unxsSalud("New tGlossary table created");
                	}
			else
			{
...
void NewtGlossary(unsigned uMode)
{
	...
	unxsSaludLog(uGlossary,"tGlossary","New");
#endif
...
void DeletetGlossary(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uGlossary,"tGlossary","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uGlossary,"tGlossary","DelError");
#endif
...
void ModtGlossary(void)
{
	...
	unxsSaludLog(uGlossary,"tGlossary","Mod");
#endif
	tGlossary(gcQuery);
