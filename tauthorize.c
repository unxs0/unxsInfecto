/*
FILE
	tAuthorize source code of unxsSalud.cgi
	Built by unxsSalud (C) Gary Wallis 2001-2012
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetAuthorize();
				unxsSalud("New tAuthorize table created");
                	}
			else
			{
...
void NewtAuthorize(unsigned uMode)
{
	...
	unxsSaludLog(uAuthorize,"tAuthorize","New");
#endif
...
void DeletetAuthorize(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uAuthorize,"tAuthorize","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uAuthorize,"tAuthorize","DelError");
#endif
...
void ModtAuthorize(void)
{
	...
	unxsSaludLog(uAuthorize,"tAuthorize","Mod");
#endif
	tAuthorize(gcQuery);
