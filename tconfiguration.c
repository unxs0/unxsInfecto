/*
FILE
	tConfiguration source code of unxsSalud.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
				...
				unxsSalud("New tConfiguration table created");
                	}
			else
			{
...
void NewtConfiguration(unsigned uMode)
{
	...
	unxsSaludLog(uConfiguration,"tConfiguration","New");
...
void DeletetConfiguration(void)
{
	...
	{
		unxsSaludLog(uConfiguration,"tConfiguration","Del");
		tConfiguration(LANG_NBR_RECDELETED);
	}
	else
	{
		unxsSaludLog(uConfiguration,"tConfiguration","DelError");
...
void ModtConfiguration(void)
{
	...
	unxsSaludLog(uConfiguration,"tConfiguration","Mod");
	if(mysql_errno(&gMysql)) htmlPlainTextError(mysql_error(&gMysql));
