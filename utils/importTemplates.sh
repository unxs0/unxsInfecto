...
#
#FILE
#	unxsSalud/utils/importTemplate.sh
#	Template unxsRAD/templates/default/bootstrap/importTemplate.sh
#PURPOSE
#	Import unxsSalud.cgi templates in one fell swoop.
...
if [ "$cunxsRAD" != "" ] && [ "$cCGIDIR" != "" ];then
	if [ "$1" == "--help" ];then
		...
		echo unxsSalud dir set via ENV unxsSalud;
	...
	if [ "$2" == "" ] || [ "$1" == "--help" ];then
		echo "usage: $0 <unxsSalud dir> <cgi-dir> [<template set>] | --help";
		echo "	or export CGIDIR=/var/www/cgi-bin && export unxsSalud=/home/user/unxsSalud";
			echo "	and then usage is only: $0 [<template set>]";
		echo "	unxsSalud dir via unxsSalud env e.g. /home/user/unxsSalud"
...
if [ -e "$cCGIDIR/unxsSalud.cgi" ]; then
	...
	cFilename=`echo $i | rev | cut -f 1 -d / | rev`;
        $cCGIDIR/unxsSalud.cgi ImportTemplateFile $cFilename $i $cTemplateSet $cTemplateType;
    done
else
	echo "unxsSalud.cgi isn't present in the CGIDIR you've defined!";
fi
