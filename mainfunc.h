...
static char cTableList[64][32]={"tPaciente","tConsulta","tMotivo","tCentro","tAntecedente","tInternacion","tMedico","tMedicacion","tInmunizacion","tTipoConsulta","tLaboratorio","tSangre","tSerologia","tMicrobiologia","tHemocultivo","tUrocultivo","tCultivoCateter","tCultivoPB","tCultivoHueso","tCultivoEsputo","tCultivoLCR","tOrina","tVirologia","tVitales","tClient","tAuthorize","tServer","tConfiguration","tStatus","tJobStatus","tJob","tGlossary","tTemplate","tTemplateSet","tTemplateType","tLog","tLogType","tLogMonth","tMonth",""};
...
void ExtMainContent(void)
{
	...
	printf("<td>Start (RADBuildInfo %s)</td></tr>\n",sgcBuildInfo);

	OpenRow("Application Summary","black");
	printf("<td>App web para para manejo simple y rápido de historial clínica.</td></tr>\n");
	...
	if(guPermLevel>9)
	{
		...
		for(i=0;cTableList[i][0];i++)
			printf("<a href=unxsSalud.cgi?gcFunction=%.32s>%.32s</a><br>\n",
...
void RestoreAll(char *cPasswd)
{
	...
	printf("Restoring unxsSalud data from .txt file in %s/unxsSalud/data...\n\n",cInstallDir);
	...
	for(i=0;cTableList[i][0];i++)
	{
sprintf(gcQuery,"LOAD DATA INFILE '%s/unxsSalud/data/%s.txt' REPLACE INTO TABLE %s",cInstallDir,cTableList[i],cTableList[i]);
		mysql_query(&gMysql,gcQuery);
		if(mysql_errno(&gMysql))
		{
			printf("%s\n",mysql_error(&gMysql));
...
void Restore(char *cPasswd, char *cTableName)
{
	...
	sprintf(gcQuery,"LOAD DATA INFILE '%s/unxsSalud/data/%s.txt' REPLACE INTO TABLE %s",cInstallDir,cTableName,cTableName);
	mysql_query(&gMysql,gcQuery);
	if(mysql_errno(&gMysql))
	{
		printf("%s\n",mysql_error(&gMysql));
...
void Backup(char *cPasswd)
{
	...
	printf("Backing up unxsSalud data to .txt files in %s/unxsSalud/data...\n\n",cInstallDir);
	...
	for(i=0;cTableList[i][0];i++)
	{
		...
		sprintf(cFileName,"%s/unxsSalud/data/%s.txt"
...
void Initialize(char *cPasswd)
{
	...
	printf("Creating db and setting permissions, installing data from %s/unxsSalud...\n\n",cInstallDir);
	...
	//Standard RAD3 required tables
        ...
        {
                sprintf(gcQuery,"LOAD DATA INFILE '%s/unxsSalud/data/%s.txt' REPLACE INTO TABLE %s",
...
void CalledByAlias(int iArgc,char *cArgv[])
{
	if(strstr(cArgv[0],"unxsSaludRSS.xml"))
	{
		MYSQL_RES *res;
		MYSQL_ROW field;
		char cRSSDate[200];
		...
		sprintf(cLinkStart,"%s://%s/cgi-bin/unxsSalud.cgi",cHTTP,gcHost);
		...
		printf("<channel>\n");
		printf("<title>unxsSalud RSS tJob Errors</title>\n");
		printf("<link>http://openisp.net/unxsSalud</link>\n");
		printf("<description>unxsSalud tJob Errors</description>\n");
		printf("<lastBuildDate>%.199s</lastBuildDate>\n",cRSSDate);
		printf("<generator>unxsSalud RSS Generator</generator>\n");
		...
		{
			printf("\n<item>\n");
			printf("<title>unxsSalud.tJob.uJob=%s</title>\n",field[0]);
...
{
	printf("\nPlease report this unxsSalud fatal error ASAP:\n%s\n",cError);
