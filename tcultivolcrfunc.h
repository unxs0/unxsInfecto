	...
	if(!strcmp(gcFunction,"tCultivoLCRTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tCultivoLCR(LANG_NB_CONFIRMNEW);
			}
			else
				tCultivoLCR("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
	                        guMode=2001;
				tCultivoLCR(LANG_NB_CONFIRMDEL);
			}
			else
				tCultivoLCR("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
				guMode=2002;
				tCultivoLCR(LANG_NB_CONFIRMMOD);
			}
			else
				tCultivoLCR("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tCultivoLCR Aux Panel",100);
	switch(guMode)
        {
                case 2000:
		...
		default:
			...
			printf("Tabla de Cultivo LCR<br>(RADBuildInfo %.32s)\n",sgcBuildInfo);
...
void ExttCultivoLCRGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uCultivoLCR"))
		{
			sscanf(gentries[i].val,"%u",&uCultivoLCR);
			guMode=6;
		}
	}
	tCultivoLCR("");
...
void ExttCultivoLCRSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tCultivoLCR ORDER BY"
				" uCultivoLCR",
				VAR_LIST_tCultivoLCR);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tCultivoLCR,tClient WHERE tCultivoLCR.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uCultivoLCR",
					VAR_LIST_tCultivoLCR,uContactParentCompany,uContactParentCompany);
...
void ExttCultivoLCRSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tCultivoLCR WHERE uCultivoLCR=%u",
			VAR_LIST_tCultivoLCR,uCultivoLCR);
	else
                sprintf(gcQuery,"SELECT %s FROM tCultivoLCR,tClient"
                                " WHERE tCultivoLCR.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tCultivoLCR.uCultivoLCR=%u",
                        		VAR_LIST_tCultivoLCR
...
void ExttCultivoLCRListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tCultivoLCR",
				VAR_LIST_tCultivoLCR);
	else
		sprintf(gcQuery,"SELECT %s FROM tCultivoLCR,tClient"
				" WHERE tCultivoLCR.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttCultivoLCRListFilter()
        if(!strcmp(gcFilter,"uCultivoLCR"))
        {
                sscanf(gcCommand,"%u",&uCultivoLCR);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tCultivoLCR.uCultivoLCR=%u"
						" ORDER BY uCultivoLCR",
						uCultivoLCR);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uCultivoLCR");
        }
...
void ExttCultivoLCRListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uCultivoLCR"))
                printf("<option>uCultivoLCR</option>");
        else
                printf("<option selected>uCultivoLCR</option>");
...
void tCultivoLCRNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uCultivoLCR,cLabel FROM tCultivoLCR ORDER BY cLabel");
	else
        ...
        {
        	printf("<p><u>tCultivoLCRNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tCultivoLCR"
				"&uCultivoLCR=%s>%s</a><br>\n",
				field[0],field[1]);
	}
