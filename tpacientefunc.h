	...
	if(!strcmp(gcFunction,"tPacienteTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tPaciente(LANG_NB_CONFIRMNEW);
			}
			else
				tPaciente("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tPaciente Aux Panel",100);
	switch(guMode)
        {
                case 2000:
		...
		default:
			...
			printf("Datos básicos de un paciente<br>(RADBuildInfo %.32s)\n",sgcBuildInfo);
...
void ExttPacienteGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uPaciente"))
		{
...
void ExttPacienteSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tPaciente ORDER BY"
				" uPaciente",
				VAR_LIST_tPaciente);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tPaciente,tClient WHERE tPaciente.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uPaciente",
					VAR_LIST_tPaciente,uContactParentCompany,uContactParentCompany);
...
void ExttPacienteSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tPaciente WHERE uPaciente=%u",
			VAR_LIST_tPaciente,uPaciente);
	else
                sprintf(gcQuery,"SELECT %s FROM tPaciente,tClient"
                                " WHERE tPaciente.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tPaciente.uPaciente=%u",
                        		VAR_LIST_tPaciente
...
void ExttPacienteListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tPaciente",
				VAR_LIST_tPaciente);
	else
		sprintf(gcQuery,"SELECT %s FROM tPaciente,tClient"
				" WHERE tPaciente.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttPacienteListFilter()
        if(!strcmp(gcFilter,"uPaciente"))
        {
                sscanf(gcCommand,"%u",&uPaciente);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tPaciente.uPaciente=%u"
						" ORDER BY uPaciente",
						uPaciente);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uPaciente");
        }
...
void ExttPacienteListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uPaciente"))
                printf("<option>uPaciente</option>");
        else
                printf("<option selected>uPaciente</option>");
...
void tPacienteNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uPaciente,cLabel FROM tPaciente ORDER BY cLabel");
	else
        ...
        {
        	printf("<p><u>tPacienteNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tPaciente"
				"&uPaciente=%s>%s</a><br>\n",
				field[0],field[1]);
	}
