	...
	if(!strcmp(gcFunction,"tLaboratorioTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tLaboratorio(LANG_NB_CONFIRMNEW);
			}
			else
				tLaboratorio("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
	                        guMode=2001;
				tLaboratorio(LANG_NB_CONFIRMDEL);
			}
			else
				tLaboratorio("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
				guMode=2002;
				tLaboratorio(LANG_NB_CONFIRMMOD);
			}
			else
				tLaboratorio("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tLaboratorio Aux Panel",100);
	switch(guMode)
        {
                case 2000:
		...
		default:
			...
			printf("Tabla de Laboratorio<br>(RADBuildInfo %.32s)\n",sgcBuildInfo);
...
void ExttLaboratorioGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uLaboratorio"))
		{
			sscanf(gentries[i].val,"%u",&uLaboratorio);
			guMode=6;
		}
	}
	tLaboratorio("");
...
void ExttLaboratorioSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tLaboratorio ORDER BY"
				" uLaboratorio",
				VAR_LIST_tLaboratorio);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tLaboratorio,tClient WHERE tLaboratorio.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uLaboratorio",
					VAR_LIST_tLaboratorio,uContactParentCompany,uContactParentCompany);
...
void ExttLaboratorioSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tLaboratorio WHERE uLaboratorio=%u",
			VAR_LIST_tLaboratorio,uLaboratorio);
	else
                sprintf(gcQuery,"SELECT %s FROM tLaboratorio,tClient"
                                " WHERE tLaboratorio.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tLaboratorio.uLaboratorio=%u",
                        		VAR_LIST_tLaboratorio
...
void ExttLaboratorioListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tLaboratorio",
				VAR_LIST_tLaboratorio);
	else
		sprintf(gcQuery,"SELECT %s FROM tLaboratorio,tClient"
				" WHERE tLaboratorio.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttLaboratorioListFilter()
        if(!strcmp(gcFilter,"uLaboratorio"))
        {
                sscanf(gcCommand,"%u",&uLaboratorio);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tLaboratorio.uLaboratorio=%u"
						" ORDER BY uLaboratorio",
						uLaboratorio);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uLaboratorio");
        }
...
void ExttLaboratorioListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uLaboratorio"))
                printf("<option>uLaboratorio</option>");
        else
                printf("<option selected>uLaboratorio</option>");
...
void tLaboratorioNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uLaboratorio,cLabel FROM tLaboratorio ORDER BY cLabel");
	else
        ...
        {
        	printf("<p><u>tLaboratorioNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tLaboratorio"
				"&uLaboratorio=%s>%s</a><br>\n",
				field[0],field[1]);
	}
