/*
FILE
	tStatus source code of unxsSalud.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetStatus();
				unxsSalud("New tStatus table created");
                	}
			else
			{
...
void NewtStatus(unsigned uMode)
{
	...
	unxsSaludLog(uStatus,"tStatus","New");
#endif
...
void DeletetStatus(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uStatus,"tStatus","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uStatus,"tStatus","DelError");
#endif
...
void ModtStatus(void)
{
	...
	unxsSaludLog(uStatus,"tStatus","Mod");
#endif
	tStatus(gcQuery);
