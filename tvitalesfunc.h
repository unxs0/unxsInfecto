	...
	if(!strcmp(gcFunction,"tVitalesTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tVitales(LANG_NB_CONFIRMNEW);
			}
			else
				tVitales("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tVitales Aux Panel",100);
	switch(guMode)
        {
                case 2000:
		...
		default:
			...
			printf("Tabla de signos vitales<br>(RADBuildInfo %.32s)\n",sgcBuildInfo);
...
void ExttVitalesGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uVitales"))
		{
			sscanf(gentries[i].val,"%u",&uVitales);
			guMode=6;
		}
	}
	tVitales("");
...
void ExttVitalesSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tVitales ORDER BY"
				" uVitales",
				VAR_LIST_tVitales);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tVitales,tClient WHERE tVitales.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uVitales",
					VAR_LIST_tVitales,uContactParentCompany,uContactParentCompany);
...
void ExttVitalesSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tVitales WHERE uVitales=%u",
			VAR_LIST_tVitales,uVitales);
	else
                sprintf(gcQuery,"SELECT %s FROM tVitales,tClient"
                                " WHERE tVitales.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tVitales.uVitales=%u",
                        		VAR_LIST_tVitales
...
void ExttVitalesListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tVitales",
				VAR_LIST_tVitales);
	else
		sprintf(gcQuery,"SELECT %s FROM tVitales,tClient"
				" WHERE tVitales.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttVitalesListFilter()
        if(!strcmp(gcFilter,"uVitales"))
        {
                sscanf(gcCommand,"%u",&uVitales);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tVitales.uVitales=%u"
						" ORDER BY uVitales",
						uVitales);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uVitales");
        }
...
void ExttVitalesListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uVitales"))
                printf("<option>uVitales</option>");
        else
                printf("<option selected>uVitales</option>");
...
void tVitalesNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uVitales,cLabel FROM tVitales ORDER BY cLabel");
	else
        ...
        {
        	printf("<p><u>tVitalesNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tVitales"
				"&uVitales=%s>%s</a><br>\n",
				field[0],field[1]);
	}
