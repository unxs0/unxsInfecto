/*
FILE
	tJobStatus source code of unxsSalud.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetJobStatus();
				unxsSalud("New tJobStatus table created");
                	}
			else
			{
...
void NewtJobStatus(unsigned uMode)
{
	...
	unxsSaludLog(uJobStatus,"tJobStatus","New");
#endif
...
void DeletetJobStatus(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uJobStatus,"tJobStatus","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsSaludLog(uJobStatus,"tJobStatus","DelError");
#endif
...
void ModtJobStatus(void)
{
	...
	unxsSaludLog(uJobStatus,"tJobStatus","Mod");
#endif
	tJobStatus(gcQuery);
