	...
	if(!strcmp(gcFunction,"tMedicacionTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tMedicacion(LANG_NB_CONFIRMNEW);
			}
			else
				tMedicacion("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
	                        guMode=2001;
				tMedicacion(LANG_NB_CONFIRMDEL);
			}
			else
				tMedicacion("<blink>Error</blink>: Denied by permissions settings");
                }
			...
			{
				guMode=2002;
				tMedicacion(LANG_NB_CONFIRMMOD);
			}
			else
				tMedicacion("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tMedicacion Aux Panel",100);
	switch(guMode)
        {
                case 2000:
		...
		default:
			...
			printf("Tabla de Medicación. Zero o mas por paciente.<br>(RADBuildInfo %.32s)\n",sgcBuildInfo);
...
void ExttMedicacionGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uMedicacion"))
		{
			sscanf(gentries[i].val,"%u",&uMedicacion);
			guMode=6;
		}
	}
	tMedicacion("");
...
void ExttMedicacionSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tMedicacion ORDER BY"
				" uMedicacion",
				VAR_LIST_tMedicacion);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tMedicacion,tClient WHERE tMedicacion.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uMedicacion",
					VAR_LIST_tMedicacion,uContactParentCompany,uContactParentCompany);
...
void ExttMedicacionSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tMedicacion WHERE uMedicacion=%u",
			VAR_LIST_tMedicacion,uMedicacion);
	else
                sprintf(gcQuery,"SELECT %s FROM tMedicacion,tClient"
                                " WHERE tMedicacion.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tMedicacion.uMedicacion=%u",
                        		VAR_LIST_tMedicacion
...
void ExttMedicacionListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tMedicacion",
				VAR_LIST_tMedicacion);
	else
		sprintf(gcQuery,"SELECT %s FROM tMedicacion,tClient"
				" WHERE tMedicacion.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttMedicacionListFilter()
        if(!strcmp(gcFilter,"uMedicacion"))
        {
                sscanf(gcCommand,"%u",&uMedicacion);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tMedicacion.uMedicacion=%u"
						" ORDER BY uMedicacion",
						uMedicacion);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uMedicacion");
        }
...
void ExttMedicacionListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uMedicacion"))
                printf("<option>uMedicacion</option>");
        else
                printf("<option selected>uMedicacion</option>");
...
void tMedicacionNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uMedicacion,cLabel FROM tMedicacion ORDER BY cLabel");
	else
        ...
        {
        	printf("<p><u>tMedicacionNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tMedicacion"
				"&uMedicacion=%s>%s</a><br>\n",
				field[0],field[1]);
	}
