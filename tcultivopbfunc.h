	...
	if(!strcmp(gcFunction,"tCultivoPBTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=9)
			{
                        	...
                        	guMode=2000;
	                        tCultivoPB(LANG_NB_CONFIRMNEW);
			}
			else
				tCultivoPB("<blink>Error</blink>: Denied by permissions settings");
                }
...
{
	OpenFieldSet("tCultivoPB Aux Panel",100);
	switch(guMode)
        {
                case 2000:
		...
		default:
			...
			printf("Tabla de Cultivo PB<br>(RADBuildInfo %.32s)\n",sgcBuildInfo);
...
void ExttCultivoPBGetHook(entry gentries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(gentries[i].name,"uCultivoPB"))
		{
			sscanf(gentries[i].val,"%u",&uCultivoPB);
			guMode=6;
		}
	}
	tCultivoPB("");
...
void ExttCultivoPBSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tCultivoPB ORDER BY"
				" uCultivoPB",
				VAR_LIST_tCultivoPB);
	else //If you own it, the company you work for owns the company that owns it,
		//you created it, or your company owns it you can at least read access it
		//select tTemplateSet.cLabel from tTemplateSet,tClient where tTemplateSet.uOwner=tClient.uClient and tClient.uOwner in (select uClient from tClient where uOwner=81 or uClient=51);
	sprintf(gcQuery,"SELECT %s FROM tCultivoPB,tClient WHERE tCultivoPB.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" ORDER BY uCultivoPB",
					VAR_LIST_tCultivoPB,uContactParentCompany,uContactParentCompany);
...
void ExttCultivoPBSelectRow(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
                sprintf(gcQuery,"SELECT %s FROM tCultivoPB WHERE uCultivoPB=%u",
			VAR_LIST_tCultivoPB,uCultivoPB);
	else
                sprintf(gcQuery,"SELECT %s FROM tCultivoPB,tClient"
                                " WHERE tCultivoPB.uOwner=tClient.uClient"
				" AND tClient.uOwner IN (SELECT uClient FROM tClient WHERE uOwner=%u OR uClient=%u)"
				" AND tCultivoPB.uCultivoPB=%u",
                        		VAR_LIST_tCultivoPB
...
void ExttCultivoPBListSelect(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT %s FROM tCultivoPB",
				VAR_LIST_tCultivoPB);
	else
		sprintf(gcQuery,"SELECT %s FROM tCultivoPB,tClient"
				" WHERE tCultivoPB.uOwner=tClient.uClient"
	...
	//Changes here must be reflected below in ExttCultivoPBListFilter()
        if(!strcmp(gcFilter,"uCultivoPB"))
        {
                sscanf(gcCommand,"%u",&uCultivoPB);
		if(guPermLevel<10)
			strcat(gcQuery," AND ");
		else
			strcat(gcQuery," WHERE ");
		sprintf(cCat,"tCultivoPB.uCultivoPB=%u"
						" ORDER BY uCultivoPB",
						uCultivoPB);
		strcat(gcQuery,cCat);
        }
		...
		strcat(gcQuery," ORDER BY uCultivoPB");
        }
...
void ExttCultivoPBListFilter(void)
{
        ...
        if(strcmp(gcFilter,"uCultivoPB"))
                printf("<option>uCultivoPB</option>");
        else
                printf("<option selected>uCultivoPB</option>");
...
void tCultivoPBNavList(void)
{
	...
	if(guLoginClient==1 && guPermLevel>11)//Root can read access all
		sprintf(gcQuery,"SELECT uCultivoPB,cLabel FROM tCultivoPB ORDER BY cLabel");
	else
        ...
        {
        	printf("<p><u>tCultivoPBNavList</u><br>\n");
	...
	{	
			...
			printf("<a class=darkLink href=unxsSalud.cgi?gcFunction=tCultivoPB"
				"&uCultivoPB=%s>%s</a><br>\n",
				field[0],field[1]);
	}
